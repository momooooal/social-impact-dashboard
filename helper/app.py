...
from collector_core import (
    PLATFORMS, TOPICS, activity_catalog, app_data_dir, collect_all, config_path, git_sync,
    import_content_file, import_private_messages, install_task, load_config, load_private_rows,
    remove_task, save_config, save_private_rows, open_real_chrome, chrome_extension_dir, detect_chrome_exe
)
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.status_var = tk.StringVar(value='準備完成。v3.3 直接使用你平常 Chrome 已登入的工作階段，不需要重新登入。')
    ...
    def _build(self):
        ...
        ttk.Label(outer,text='不保存帳號密碼；直接使用你平常已登入的 Chrome。擴充套件只把可見 Insights 資料傳回本機。私訊原文只在本機分析。').pack(anchor='w',pady=(2,12))
    ...
    def _build_setup(self):
        ttk.Label(self.tab_setup,text='A. 直接使用你現在的 Chrome（只需設定一次）',font=('Microsoft JhengHei UI',13,'bold')).grid(row=0,column=0,columnspan=4,sticky='w',pady=(0,8))
        ttk.Label(self.tab_setup,text='你不需要重新登入。先在 Chrome 載入 v3.3 的橋接擴充套件；之後小助手會直接開你平常 Chrome 的 Insights 頁面。').grid(row=1,column=0,columnspan=4,sticky='w',pady=(0,8))
        tools=ttk.Frame(self.tab_setup); tools.grid(row=2,column=0,columnspan=4,sticky='w',pady=(0,12))
        ttk.Button(tools,text='① 開啟擴充套件資料夾',command=self.open_extension_dir).pack(side='left',padx=(0,6))
        ttk.Button(tools,text='② 開啟 Chrome 擴充功能頁',command=self.open_chrome_extensions).pack(side='left',padx=6)
        ttk.Label(tools,text='在 Chrome 開啟「開發人員模式」→「載入未封裝項目」→ 選 chrome-extension 資料夾。',foreground='#555').pack(side='left',padx=10)
        r=3
        ttk.Label(self.tab_setup,text='B. 三平台 Insights 網址',font=('Microsoft JhengHei UI',13,'bold')).grid(row=r,column=0,columnspan=4,sticky='w',pady=(6,4)); r+=1
        ttk.Label(self.tab_setup,text='網址可直接貼你平常 Chrome 已登入後看到的 Insights 頁。先用預設網址也可以，測試後再修正。').grid(row=r,column=0,columnspan=4,sticky='w',pady=(0,8)); r+=1
        for p in PLATFORMS:
            ttk.Label(self.tab_setup,text=LABELS[p],width=12).grid(row=r,column=0,sticky='w',pady=5)
            ttk.Entry(self.tab_setup,textvariable=self.url_vars[p],width=66).grid(row=r,column=1,columnspan=2,sticky='ew',padx=6)
            ttk.Button(self.tab_setup,text='用目前 Chrome 開啟',command=lambda x=p:self.open_current_chrome(x)).grid(row=r,column=3,padx=4)
            r+=1
        self.tab_setup.columnconfigure(1,weight=1); self.tab_setup.columnconfigure(2,weight=1)
        ttk.Separator(self.tab_setup).grid(row=r,column=0,columnspan=4,sticky='ew',pady=14); r+=1
        ttk.Label(self.tab_setup,text='C. GitHub Dashboard 同步（選用）',font=('Microsoft JhengHei UI',13,'bold')).grid(row=r,column=0,columnspan=4,sticky='w'); r+=1
        ...
        ttk.Label(self.tab_setup,text='用逗號分隔；用來排除自己回覆。',foreground='#666').grid(row=r+1,column=1,columnspan=3,sticky='w',padx=6); r+=2
    ...
    def _build_status(self):
        ...
        ttk.Button(top,text='▶ 從目前 Chrome 擷取三平台',command=self.collect_now).pack(side='left',padx=(0,6))
        ttk.Button(top,text='開啟 Chrome 擴充功能頁',command=self.open_chrome_extensions).pack(side='left',padx=6)
        ...
        self.log.insert('end','v3.3 直接使用你平常 Chrome 已登入的狀態。\n第一次先確認橋接擴充套件已載入，再按「從目前 Chrome 擷取三平台」。\n')
    ...
    def open_extension_dir(self):
        p=chrome_extension_dir()
        if os.name=='nt': os.startfile(str(p))
        elif sys.platform=='darwin': subprocess.Popen(['open',str(p)])
        else: subprocess.Popen(['xdg-open',str(p)])
        self.status_var.set('已開啟 chrome-extension 資料夾。請到 Chrome 擴充功能頁載入它。')

    def open_chrome_extensions(self):
        try:
            exe=detect_chrome_exe(self.cfg)
            subprocess.Popen([exe,'chrome://extensions/'])
            self.status_var.set('已開啟 Chrome 擴充功能頁。請開啟「開發人員模式」後載入未封裝項目。')
        except Exception as e:
            messagebox.showerror('無法開啟 Chrome',str(e))

    def open_current_chrome(self,p):
        self.save_settings()
        u=self.url_vars[p].get().strip()
        if not u:
            messagebox.showwarning('尚未設定網址','請先貼入 Insights 網址。'); return
        try:
            open_real_chrome(u,self.cfg)
            self.status_var.set(f'已用目前 Chrome 開啟 {LABELS[p]}。如果帳號或頁面不對，切到正確 Insights 後把網址列複製回來即可。')
        except Exception as e:
