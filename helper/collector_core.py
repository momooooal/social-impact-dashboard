...
def chrome_extension_dir() -> Path:
    return project_dir() / "chrome-extension"


def detect_chrome_exe(cfg: dict[str, Any] | None = None) -> str:
    cfg = cfg or load_config()
    explicit = str(cfg.get("chrome_exe", "")).strip()
...
def open_real_chrome(url: str, cfg: dict[str, Any] | None = None) -> None:
    cfg = cfg or load_config()
    exe = detect_chrome_exe(cfg)
    args = [exe]
    profile = str(cfg.get("chrome_profile", "")).strip()
    if profile:
    ...
    try:
        ...
        # Fall back to the system URL handler; on most Windows setups this is Chrome.
        if os.name == "nt":
...
class _BridgeState:
    ...
    def prepare(self, platform: str) -> None:
        with self.lock:
    ...
    def add(self, payload: dict[str, Any]) -> None:
        with self.lock:
            payload["_expected_platform"] = self.expected_platform
            self.records.append(payload)
            self.last_received = time.time()

    def snapshot(self) -> tuple[list[dict[str, Any]], float]:
        with self.lock:
            return list(self.records), self.last_received


class _BridgeHandler(http.server.BaseHTTPRequestHandler):
    server_version = "SocialImpactBridge/3.3"
    ...
    def do_POST(self):  # noqa: N802
        ...
        try:
            n = min(int(self.headers.get("Content-Length", "0") or 0), 8_000_000)
            ...
            if len(text) > 6_000_000:
                payload["text"] = text[:6_000_000]
            self.server.bridge_state.add(payload)  # type: ignore[attr-defined]
            body = b'{"ok":true}'
            self.send_response(200)
            self._cors()
            self.send_header("Content-Type", "application/json")
...
def _start_bridge() -> tuple[_ThreadingHTTPServer, _BridgeState, threading.Thread]:
    state = _BridgeState()
    server = _ThreadingHTTPServer((BRIDGE_HOST, BRIDGE_PORT), _BridgeHandler)
...
def _best_bridge_record(records: list[dict[str, Any]]) -> tuple[dict[str, Any] | None, dict[str, Any]]:
    best = None
    ...
    for rec in records:
        ...
        score = (len(metrics), len(text))
        if score > best_score:
            best, best_metrics, best_score = rec, metrics, score
    return best, best_metrics


def collect_platform_from_real_chrome(platform: str, cfg: dict[str, Any], state: _BridgeState) -> dict[str, Any]:
    pcfg = cfg["platforms"][platform]
    url = str(pcfg.get("insights_url", "")).strip()
    if not url:
    ...
    state.prepare(platform)
    try:
        open_real_chrome(url, cfg)
    except Exception as e:
        return {"ok": False, "message": f"無法開啟 Chrome：{e}", "metrics": {}, "captured_at": stamp}

    # Wait for the extension to send one or more progressively richer SPA snapshots.
    deadline = time.time() + 24
    had_record = False
    while time.time() < deadline:
        records, last = state.snapshot()
        if records:
            had_record = True
            # Once the last snapshot has been stable for 4s and we have waited at least a little,
            # use the richest record instead of always waiting the full timeout.
            if last and time.time() - last >= 4 and len(records) >= 2:
                break
        time.sleep(0.5)
    records, _ = state.snapshot()
    if not had_record or not records:
        return {
            "ok": False,
            "message": "未收到 Chrome 擴充套件資料；請確認擴充套件已載入、Chrome 已登入，且此頁可正常看到 Insights。",
            "metrics": {}, "captured_at": stamp, "url": url,
        }
    rec, metrics = _best_bridge_record(records)
    text = str((rec or {}).get("text", ""))
    final_url = str((rec or {}).get("url", url))
    ...
    if _login_required(text, final_url):
        return {"ok": False, "message": "目前 Chrome 的登入狀態已失效，請在平常使用的 Chrome 重新登入。", "metrics": {}, "captured_at": stamp, "url": final_url}
    if not metrics:
        return {"ok": False, "message": "Chrome 頁面有傳回資料，但尚未辨識到成效指標；請確認網址是 Insights／洞察報告頁。", "metrics": {}, "captured_at": stamp, "url": final_url}
    return {"ok": True, "message": f"從目前 Chrome 擷取 {len(metrics)} 個指標", "metrics": metrics, "url": final_url, "captured_at": stamp}


def collect_all_via_real_chrome(do_git_sync: bool | None = None) -> dict[str, Any]:
    cfg = load_config()
    data = load_output_data(cfg)
    results: dict[str, Any] = {}
    try:
        server, state, _ = _start_bridge()
    except OSError as e:
        return {"platforms": {}, "git": {"ok": False, "message": ""}, "path": str(output_data_path(cfg)), "error": f"本機橋接埠 {BRIDGE_PORT} 無法啟動：{e}"}
    try:
        for platform in PLATFORMS:
            if not cfg.get("platforms", {}).get(platform, {}).get("enabled", True):
                continue
            result = collect_platform_from_real_chrome(platform, cfg, state)
...
def collect_all(headed: bool = False, do_git_sync: bool | None = None) -> dict[str, Any]:
    cfg = load_config()
    if cfg.get("collection_mode", "chrome-extension") == "chrome-extension":
        return collect_all_via_real_chrome(do_git_sync=do_git_sync)
    data = load_output_data(cfg)
    results = {}
    for platform in PLATFORMS:
        if not cfg.get("platforms", {}).get(platform, {}).get("enabled", True):
